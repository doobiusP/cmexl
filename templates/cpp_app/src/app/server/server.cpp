...
#else
  ssize_t ret = write(connfd, wbuf, strlen(wbuf));
  if (ret < 0) {
    perror("write failed");
  } else if ((size_t)ret < strlen(wbuf)) {
    fprintf(stderr, "Warning: partial write occurred (%zd of %zu bytes)\n", ret,
            strlen(wbuf));
  }
#endif
}
...
int main() {
  ...
  while (true) {
    ...
    socket_t connfd = accept(fd, (sockaddr *)&client_addr, &addrlen);
